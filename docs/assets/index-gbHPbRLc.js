var Rt=Object.defineProperty;var ct=s=>{throw TypeError(s)};var zt=(s,t,e)=>t in s?Rt(s,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):s[t]=e;var k=(s,t,e)=>zt(s,typeof t!="symbol"?t+"":t,e),ut=(s,t,e)=>t.has(s)||ct("Cannot "+e);var p=(s,t,e)=>(ut(s,t,"read from private field"),e?e.call(s):t.get(s)),N=(s,t,e)=>t.has(s)?ct("Cannot add the same private member more than once"):t instanceof WeakSet?t.add(s):t.set(s,e),et=(s,t,e,i)=>(ut(s,t,"write to private field"),i?i.call(s,e):t.set(s,e),e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))i(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const a of r.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&i(a)}).observe(document,{childList:!0,subtree:!0});function e(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(o){if(o.ep)return;o.ep=!0;const r=e(o);fetch(o.href,r)}})();class $t extends HTMLElement{static get observedAttributes(){return["color","size"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host{
            font-size:inherit;
            display:inline-flex;
        ...
        <slot></slot>
        `}get size(){return this.getAttribute("size")||""}get color(){return this.getAttribute("color")||""}set size(t){this.setAttribute("size",t)}set color(t){this.setAttribute("color",t)}connectedCallback(){this.loading=this.shadowRoot.getElementById("loading"),this.size&&(this.size=this.size),this.color&&(this.color=this.color)}attributeChangedCallback(t,e,i){t=="color"&&this.loading&&(this.loading.style.color=i),t=="size"&&this.loading&&(this.loading.style.fontSize=i+"px")}}customElements.get("xy-loading")||customElements.define("xy-loading",$t);class St extends HTMLElement{static get observedAttributes(){return["name","size","color","path"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host{
            font-size:inherit;
        ...
        */
        ...
        <slot></slot>
        `}get disabled(){return this.getAttribute("disabled")!==null}set disabled(t){t===null||t===!1?this.removeAttribute("disabled"):this.setAttribute("disabled","")}connectedCallback(){}attributeChangedCallback(t,e,i){}}customElements.get("xy-button-group")||customElements.define("xy-button-group",Bt);class Tt extends HTMLElement{static get observedAttributes(){return["color"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host {
            display:inline-block;
            position: relative;
        ...
        <slot></slot>
        `}get label(){return this.getAttribute("label")}}customElements.get("xy-optgroup")||customElements.define("xy-optgroup",Dt);class Ht extends HTMLElement{static get observedAttributes(){return["value","selected","disabled"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
            :host{
                display: block;
            }
        ...
        */
        ...
        <slot></slot>
        `}get gutter(){return this.getAttribute("gutter")}set gutter(t){this.setAttribute("gutter",t)}attributeChangedCallback(t,e,i){t=="gutter"&&this.shadowRoot&&this.style.setProperty("--gutter",i+"px")}}customElements.get("xy-row")||customElements.define("xy-row",Ut);class Gt extends HTMLElement{constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host {
            grid-column: span 1;
        }
...
`).join("")}
        ...
        <slot></slot>
        `}}customElements.get("xy-col")||customElements.define("xy-col",Gt);class Wt extends HTMLElement{constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host {
            display:flex;
            flex-direction:column;
        ...
        <slot></slot>
        `}}customElements.get("xy-layout")||customElements.define("xy-layout",Wt);class gt extends HTMLElement{constructor({multi:e}={}){super();k(this,"setlist",e=>{this.list&&(this.contains(e.target)||this.list.contains(e.target)?this.list.show=!0:this.list.show=!1)});this.multi=e,this.$customValidity=null;const i=this.attachShadow({mode:"open"});i.innerHTML=`
        <style>
        :host{
            box-sizing:border-box;
            display:inline-block;
        ...
        <slot></slot>
        `}}customElements.get("xy-input-group")||customElements.define("xy-input-group",Kt);class mt extends HTMLElement{static get observedAttributes(){return["open"]}constructor(){super();const t=this.attachShadow({mode:"open"});this.setAttribute("tabindex",0),t.innerHTML=`
        <style>
        :host{
            position:fixed;
            display:flex;
        ...
        `}get show(){return this.getAttribute("show")!==null}get icon(){return this.getAttribute("icon")}get type(){return this.getAttribute("type")}set type(t){this.setAttribute("type",t)}set icon(t){this.setAttribute("icon",t)}set show(t){t===null||t===!1?this.removeAttribute("show"):this.setAttribute("show","")}typeMap(t){let e="",i="";switch(t){case"info":e="info-circle-fill",i="var(--infoColor,#1890ff)";break;case"success":e="check-circle-fill",i="var(--successColor,#52c41a)";break;case"error":e="close-circle-fill",i="var(--errorColor,#f4615c)";break;case"warning":e="warning-circle-fill",i="var(--waringColor,#faad14)";break}return{name:e,color:i}}connectedCallback(){this.remove=!1,this.messageType=this.shadowRoot.getElementById("message-type"),this.shadowRoot.addEventListener("transitionend",t=>{t.propertyName==="transform"&&!this.show&&(B.removeChild(this),this.dispatchEvent(new CustomEvent("close")))}),this.type=this.type,this.clientWidth}attributeChangedCallback(t,e,i){t=="type"&&this.messageType&&i!==null&&(this.messageType.name=this.typeMap(i).name,this.messageType.color=this.typeMap(i).color),t=="icon"&&this.messageType&&i!==null&&(this.messageType.name=i)}};customElements.get("xy-message")||customElements.define("xy-message",U);let B=document.getElementById("message-content");B||(B=document.createElement("div"),B.id="message-content",B.style="position:fixed; pointer-events:none; left:0; right:0; top:10px; z-index:51;",document.body.appendChild(B));const J={info:function(s="",t,e){const i=new U;return i.timer&&clearTimeout(i.timer),B.appendChild(i),i.type="info",i.textContent=s,i.show=!0,i.onclose=e,i.timer=setTimeout(()=>{i.show=!1},t||3e3),i},success:function(s="",t,e){const i=new U;return i.timer&&clearTimeout(i.timer),B.appendChild(i),i.type="success",i.textContent=s,i.show=!0,i.onclose=e,i.timer=setTimeout(()=>{i.show=!1},t||3e3),i},error:function(s="",t,e){const i=new U;return i.timer&&clearTimeout(i.timer),B.appendChild(i),i.type="error",i.textContent=s,i.show=!0,i.onclose=e,i.timer=setTimeout(()=>{i.show=!1},t||3e3),i},warning:function(s="",t,e){const i=new U;return i.timer&&clearTimeout(i.timer),B.appendChild(i),i.type="warning",i.textContent=s,i.show=!0,i.onclose=e,i.timer=setTimeout(()=>{i.show=!1},t||3e3),i},loading:function(s="",t=0,e){const i=new U;return i.timer&&clearTimeout(i.timer),B.appendChild(i),i.type="loading",i.textContent=s,i.show=!0,i.onclose=e,t!==0&&(i.timer=setTimeout(()=>{i.show=!1},t||3e3)),i},show:function({text:s,duration:t,onclose:e,icon:i}){const o=new U;return o.timer&&clearTimeout(o.timer),B.appendChild(o),o.icon=i,o.textContent=s||"",o.show=!0,o.onclose=e,t!==0&&(o.timer=setTimeout(()=>{o.show=!1},t||3e3)),o}},{min:it,max:Qt,floor:te,round:ee}=Math;function ie(s){if(s.toLowerCase()==="black")return"#000000";const t=document.createElement("canvas").getContext("2d");return t.fillStyle=s,t.fillStyle==="#000000"?null:t.fillStyle}function ht(s,t,e){s=s/360*6,t/=100,e/=100;let i=te(s),o=s-i,r=e*(1-t),a=e*(1-o*t),n=e*(1-(1-o)*t),h=i%6,d=[e,a,r,r,n,e][h],c=[n,e,e,a,r,r][h],b=[r,r,n,e,e,a][h];return[d*255,c*255,b*255]}function se(s,t,e){return ht(s,t,e).map(i=>ee(i).toString(16).padStart(2,"0"))}function oe(s,t,e){const i=ht(s,t,e),o=i[0]/255,r=i[1]/255,a=i[2]/255;let n,h,d,c;return n=it(1-o,1-r,1-a),h=n===1?0:(1-o-n)/(1-n),d=n===1?0:(1-r-n)/(1-n),c=n===1?0:(1-a-n)/(1-n),[h*100,d*100,c*100,n*100]}function re(s,t,e){t/=100,e/=100;let i=(2-t)*e/2;return i!==0&&(i===1?t=0:i<.5?t=t*e/(i*2):t=t*e/(2-i*2)),[s,t*100,i*100]}function dt(s,t,e){s/=255,t/=255,e/=255;let i,o,r;const a=it(s,t,e),n=Qt(s,t,e),h=n-a;if(r=n,h===0)i=o=0;else{o=h/n;let d=((n-s)/6+h/2)/h,c=((n-t)/6+h/2)/h,b=((n-e)/6+h/2)/h;s===n?i=b-c:t===n?i=1/3+d-b:e===n&&(i=2/3+c-d),i<0?i+=1:i>1&&(i-=1)}return[i*360,o*100,r*100]}function ne(s,t,e,i){s/=100,t/=100,e/=100,i/=100;const o=(1-it(1,s*(1-i)+i))*255,r=(1-it(1,t*(1-i)+i))*255,a=(1-it(1,e*(1-i)+i))*255;return[...dt(o,r,a)]}function ae(s,t,e){t/=100,e/=100,t*=e<.5?e:1-e;let i=e+t?2*t/(e+t)*100:0,o=(e+t)*100;return[s,i,o]}function le(s){return dt(...s.match(/.{2}/g).map(t=>parseInt(t,16)))}function yt(s){s=s.match(/^[a-zA-Z]+$/)?ie(s):s;const t={cmyk:/^cmyk[\D]+([\d.]+)[\D]+([\d.]+)[\D]+([\d.]+)[\D]+([\d.]+)/i,rgba:/^((rgba)|rgb)[\D]+([\d.]+)[\D]+([\d.]+)[\D]+([\d.]+)[\D]*?([\d.]+|$)/i,hsla:/^((hsla)|hsl)[\D]+([\d.]+)[\D]+([\d.]+)[\D]+([\d.]+)[\D]*?([\d.]+|$)/i,hsva:/^((hsva)|hsv)[\D]+([\d.]+)[\D]+([\d.]+)[\D]+([\d.]+)[\D]*?([\d.]+|$)/i,hexa:/^#?(([\dA-Fa-f]{3,4})|([\dA-Fa-f]{6})|([\dA-Fa-f]{8}))$/i},e=o=>o.map(r=>/^(|\d+)\.\d+|\d+$/.test(r)?Number(r):void 0);let i;t:for(const o in t)if(i=t[o].exec(s))switch(o){case"cmyk":{let[,r,a,n,h]=e(i);if(r>100||a>100||n>100||h>100)break t;return{values:ne(r,a,n,h),type:o}}case"rgba":{let[,,,r,a,n,h]=e(i);if(r>255||a>255||n>255||h<0||h>1)break t;return{values:[...dt(r,a,n),h],a:h,type:o}}case"hexa":{let[,r]=i;(r.length===4||r.length===3)&&(r=r.split("").map(h=>h+h).join(""));const a=r.substring(0,6);let n=r.substring(6);return n=n?parseInt(n,16)/255:void 0,{values:[...le(a),n],a:n,type:o}}case"hsla":{let[,,,r,a,n,h]=e(i);if(r>360||a>100||n>100||h<0||h>1)break t;return{values:[...ae(r,a,n),h],a:h,type:o}}case"hsva":{let[,,,r,a,n,h]=e(i);if(r>360||a>100||n>100||h<0||h>1)break t;return{values:[r,a,n,h],a:h,type:o}}}return{values:null,type:null}}function G(s=0,t=0,e=0,i=1){const o=(a,n)=>(h=0)=>n(~h?a.map(d=>Number(d.toFixed(h))):a),r={h:s,s:t,v:e,a:i,toHSVA(){const a=[r.h,r.s,r.v,r.a];return a.toString=o(a,n=>`hsva(${n[0]}, ${n[1]}%, ${n[2]}%, ${r.a})`),a},toHSLA(){const a=[...re(r.h,r.s,r.v),r.a];return a.toString=o(a,n=>`hsla(${n[0]}, ${n[1]}%, ${n[2]}%, ${r.a})`),a},toRGBA(){const a=[...ht(r.h,r.s,r.v),r.a];return a.toString=o(a,n=>`rgba(${n[0]}, ${n[1]}, ${n[2]}, ${r.a})`),a},toCMYK(){const a=oe(r.h,r.s,r.v);return a.toString=o(a,n=>`cmyk(${n[0]}%, ${n[1]}%, ${n[2]}%, ${n[3]}%)`),a},toHEXA(){const a=se(r.h,r.s,r.v),n=r.a>=1?"":Number((r.a*255).toFixed(0)).toString(16).toUpperCase().padStart(2,"0");return n&&a.push(n),a.toString=()=>`#${a.join("").toUpperCase()}`,a},clone:()=>G(r.h,r.s,r.v,r.a)};return r}const he=["#f44336","#E91E63","#9C27B0","#673AB7","#3F51B5","#2196F3","#03A9F4","#00BCD4","#009688","#4CAF50","#8BC34A","#CDDC39","#FFEB3B","#FFC107","#FF9800","#FF5722","#795548","#9E9E9E","#607D8B","rgba(0,0,0,.65)","transparent"];class vt extends HTMLElement{constructor(){super();k(this,"mousemove",e=>{this.start&&this.choose(e)});k(this,"mouseup",()=>{getComputedStyle(this.palette).opacity!==1&&this.start&&this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value,color:this.color}})),this.start=!1});const e=this.attachShadow({mode:"open"});e.innerHTML=`
        <style>
            :host{
                display: block;
                min-width: 300px;
        ...
        <div class="color-pane" id="color-pane">
            ...
            <div class="color-sign" id="colors">
                ${he.map(i=>'<button style="background-color:'+i+'" data-color='+i+"></button>").join("")}
            </div>
        </div>
        `}choose(e){const{x:i,y:o,width:r,height:a}=this.palette.getBoundingClientRect(),n=[...this.$value],h=Math.min(Math.max(0,(e.clientX-i)/r*100),100),d=Math.min(Math.max(0,(e.clientY-o)/a*100),100);n[1]=h,n[2]=100-d,this.value=`hsva(${n[0]}, ${n[1]}%, ${n[2]}%, ${n[3]})`}connectedCallback(){this.type=["HEXA","RGBA","HSLA"],this.typeindex=0,this.palette=this.shadowRoot.getElementById("color-palette"),this.colors=this.shadowRoot.getElementById("colors"),this.pane=this.shadowRoot.getElementById("color-pane"),this.rangeHue=this.shadowRoot.getElementById("range-hue"),this.rangeOpacity=this.shadowRoot.getElementById("range-opacity"),this.copyBtn=this.shadowRoot.getElementById("copy-btn"),this.copyinfo=this.copyBtn.querySelector("input"),this.switch=this.shadowRoot.getElementById("btn-switch"),this.colorHexa=this.shadowRoot.getElementById("color-hexa").querySelectorAll("input"),this.colorRgba=this.shadowRoot.getElementById("color-rgba").querySelectorAll("input"),this.colorHlsa=this.shadowRoot.getElementById("color-hlsa").querySelectorAll("input"),this.value=this.defaultvalue,this.rangeHue.addEventListener("input",()=>{const e=[...this.$value];e[0]=Number(this.rangeHue.value),this.nativeclick=!0,this.value=`hsva(${e[0]}, ${e[1]}%, ${e[2]}%, ${e[3]})`}),this.palette.addEventListener("mousedown",e=>{this.start=!0,this.choose(e)}),document.addEventListener("mousemove",this.mousemove),document.addEventListener("mouseup",this.mouseup),this.rangeOpacity.addEventListener("input",()=>{const e=[...this.$value];e[3]=Number(this.rangeOpacity.value),this.nativeclick=!0,this.value=`hsva(${e[0]}, ${e[1]}%, ${e[2]}%, ${e[3]})`}),this.colors.addEventListener("click",e=>{const i=e.target.closest("button");i&&(this.nativeclick=!0,this.value=i.dataset.color)}),this.switch.addEventListener("click",()=>{this.typeindex++,this.typeindex%=3,this.switch.textContent=this.type[this.typeindex],this.nativeclick=!0,this.value=this.value,this.switch.parentNode.dataset.type=this.type[this.typeindex]}),this.copyBtn.addEventListener("click",()=>{this.copyinfo.select(),document.execCommand("copy")&&(document.execCommand("copy"),J.success(this.value))}),this.colorHexa.forEach(e=>{e.addEventListener("change",()=>{this.nativeclick=!0,this.value=e.value})}),this.colorRgba.forEach((e,i)=>{e.addEventListener("change",()=>{const o=G(...this.$value).toRGBA();o[i]=Number(e.value),this.nativeclick=!0,this.value=`rgba(${o[0]}, ${o[1]}, ${o[2]}, ${o[3]})`})}),this.colorHlsa.forEach((e,i)=>{e.addEventListener("change",()=>{const o=G(...this.$value).toHSLA();o[i]=Number(e.value),this.nativeclick=!0,this.value=`hsla(${o[0]}, ${o[1]}%, ${o[2]}%, ${o[3]})`})})}disconnectedCallback(){document.removeEventListener("mousemove",this.mousemove),document.removeEventListener("mouseup",this.mouseup)}get value(){return G(...this.$value)["to"+this.type[this.typeindex]]().toString()}get color(){return G(...this.$value)}get defaultvalue(){return this.getAttribute("defaultvalue")||"#ff0000"}set defaultvalue(e){this.setAttribute("defaultvalue",e)}set value(e){this.$value=yt(e).values;const[i,o,r,a=1]=this.$value;this.pane.style.setProperty("--h",i),this.pane.style.setProperty("--s",o),this.pane.style.setProperty("--v",r),this.pane.style.setProperty("--a",a),this.pane.style.setProperty("--c",this.value),this.copyinfo.value=this.value,this.rangeHue.value=i,this.rangeOpacity.value=a.toFixed(2);const n=G(...this.$value);this.colorHexa[0].value=n.toHEXA().toString();const h=n.toRGBA();this.colorRgba[0].value=h[0].toFixed(0),this.colorRgba[1].value=h[1].toFixed(0),this.colorRgba[2].value=h[2].toFixed(0),this.colorRgba[3].value=h[3].toFixed(2);const d=n.toHSLA();this.colorHlsa[0].value=d[0].toFixed(0),this.colorHlsa[1].value=d[1].toFixed(0),this.colorHlsa[2].value=d[2].toFixed(0),this.colorHlsa[3].value=d[3].toFixed(2),this.nativeclick&&(this.nativeclick=!1,this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value,color:this.color}})))}}customElements.get("xy-color-pane")||customElements.define("xy-color-pane",vt);class de extends HTMLElement{static get observedAttributes(){return["disabled","dir"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host{
            display:inline-block;
            width:30px;
        ...
        `}focus(){this.colorBtn.focus()}connectedCallback(){this.popover=this.shadowRoot.getElementById("popover"),this.popcon=this.shadowRoot.getElementById("popcon"),this.colorBtn=this.shadowRoot.getElementById("color-btn"),this.btnSubmit=this.shadowRoot.getElementById("btn-submit"),this.colorBtn.addEventListener("click",()=>{this.colorPane||(this.colorPane=new vt,this.colorPane.defaultvalue=this.defaultvalue,this.popcon.prepend(this.colorPane))}),this.btnSubmit.addEventListener("click",()=>{this.nativeclick=!0,this.value=this.colorPane.value}),this.popcon.addEventListener("close",()=>{this.colorPane.value=this.value}),this.value=this.defaultvalue}get defaultvalue(){return this.getAttribute("defaultvalue")||"#42b983"}get value(){return this.$value}get color(){return G(...yt(this.$value).values)}get type(){return this.getAttribute("type")}get disabled(){return this.getAttribute("disabled")!==null}get dir(){return this.getAttribute("dir")}set dir(t){this.setAttribute("dir",t)}set disabled(t){t===null||t===!1?this.removeAttribute("disabled"):this.setAttribute("disabled","")}set defaultvalue(t){this.setAttribute("defaultvalue",t)}set value(t){this.colorBtn.style.setProperty("--themeColor",t),this.$value=t,this.nativeclick?(this.nativeclick=!1,this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value,color:this.color}}))):this.colorPane?this.colorPane.value=this.value:this.defaultvalue=this.value}attributeChangedCallback(t,e,i){t=="disabled"&&this.colorBtn&&(i!=null?this.colorBtn.setAttribute("disabled","disabled"):this.colorBtn.removeAttribute("disabled")),t=="dir"&&this.popover&&i!=null&&(this.popover.dir=i)}}customElements.get("xy-color-picker")||customElements.define("xy-color-picker",de);class ce extends HTMLElement{static get observedAttributes(){return["disabled"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host {
            display:block;
        }
        ...
        `}get defaultvalue(){return this.getAttribute("defaultvalue")||new Date}get range(){return this.getAttribute("range")}get min(){const t=this.getAttribute("min"),e=[0,1,1];return e.default=!0,t?P(t):e}get max(){const t=this.getAttribute("max"),e=[9999,12,31];return e.default=!0,t?P(t):e}get minormax(){return!this.min.default||!this.max.default}set defaultvalue(t){this.setAttribute("defaultvalue",t)}getDays(t,e){const i=new Date(t,e-1,0).getDate(),o=new Date(t,e,0).getDate(),r=new Date(t,e-1,1).getDay(),a=Array.from({length:r},(d,c)=>(e==1?t-1:t)+"-"+(e==1?12:e-1)+"-"+(i+c-r+1)),n=Array.from({length:o},(d,c)=>t+"-"+e+"-"+(c+1)),h=Array.from({length:42-o-r},(d,c)=>(e==12?t+1:t)+"-"+(e==12?1:e+1)+"-"+(c+1));return[...a,...n,...h]}getMonths(){return["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"]}getYears(t){const e=parseInt(t/20)*20;return Array.from({length:20},(i,o)=>e+o)}toDay(t,e,i){const o=new Date(t,e+1,0).getDate();return i=i>o?o:i,[t,e,i]}select(t){this.dispatchEvent(new CustomEvent("select",{detail:{value:t,date:this.date}}))}render(t=this.$value){this.$value=t;const[e,i,o]=P(t),[r,a,n]=P(new Date),h=this.range?this.previousElementSibling:null,d=this.range?this.nextElementSibling:null;switch(this.mode){case"date":const c=this.getDays(e,i);this.days.forEach((l,m)=>{const[S,I,D]=c[m].split("-");l.dataset.date=S+"-"+I.toString().padStart(2,0)+"-"+D.toString().padStart(2,0),l.dataset.year=S,l.dataset.month=I.toString().padStart(2,0),l.dataset.day=D.toString().padStart(2,0),l.textContent=D,r+"-"+a+"-"+n==c[m]?l.setAttribute("now",""):l.removeAttribute("now"),I!=i?l.setAttribute("other",""):l.removeAttribute("other"),this.minormax?l.disabled=l.dataset.date<u(this.min)||l.dataset.date>u(this.max):l.disabled=!1,this.range?(l.dataset.date>u(this.rangedate[0])&&l.dataset.date<u(this.rangedate[1])?l.setAttribute("select",""):l.removeAttribute("select"),l.dataset.date==u(this.rangedate[0])?l.setAttribute("selectstart",""):l.removeAttribute("selectstart"),l.dataset.date==u(this.rangedate[1])?l.setAttribute("selectend",""):l.removeAttribute("selectend"),(this.range==="left"&&d&&u(l.dataset.date,"month")>u(d.value,"month")||this.range==="right"&&h&&u(l.dataset.date,"month")<u(h.value,"month"))&&(l.disabled=!0)):e+"-"+i+"-"+o==c[m]?l.setAttribute("current",""):l.removeAttribute("current")}),this.switch.textContent=e+"年"+(i+"").padStart(2,0)+"月",this.switch.disabled=!1,this.minormax?(this.prev.disabled=u(this.min,"month")>=u(t,"month"),this.next.disabled=u(this.max,"month")<=u(t,"month")):(this.prev.disabled=!1,this.next.disabled=!1),this.range==="left"&&u(t,"month")>=u(d.value,"month")&&(this.next.disabled=!0),this.range==="right"&&u(t,"month")<=u(h.value,"month")&&(this.prev.disabled=!0);break;case"month":if(this.months.forEach((l,m)=>{l.dataset.date=e+"-"+l.dataset.month,l.dataset.year=e,r+"-"+a==e+"-"+Number(l.dataset.month)?l.setAttribute("now",""):l.removeAttribute("now"),this.minormax?l.disabled=l.dataset.date<u(this.min,"month")||l.dataset.date>u(this.max,"month"):l.disabled=!1,this.range?(l.dataset.date>u(this.rangedate[0],"month")&&l.dataset.date<u(this.rangedate[1],"month")?l.setAttribute("select",""):l.removeAttribute("select"),l.dataset.date==u(this.rangedate[0],"month")?l.setAttribute("selectstart",""):l.removeAttribute("selectstart"),l.dataset.date==u(this.rangedate[1],"month")?l.setAttribute("selectend",""):l.removeAttribute("selectend"),this.type=="date"&&(this.range==="left"&&u(l.dataset.date,"month")>u(d.value,"month")||this.range==="right"&&u(l.dataset.date,"month")<u(h.value,"month"))&&(l.disabled=!0)):l.dataset.month==i?l.setAttribute("current",""):l.removeAttribute("current")}),this.switch.textContent=e+"年",this.switch.disabled=!1,this.minormax?(this.prev.disabled=this.min[0]>=e,this.next.disabled=this.max[0]<=e):(this.prev.disabled=!1,this.next.disabled=!1),this.range==="left"){const l=this.nextElementSibling;e>=u(l.value,"year")&&(this.next.disabled=!0)}if(this.range==="right"){const l=this.previousElementSibling;e<=u(l.value,"year")&&(this.prev.disabled=!0)}break;case"year":const b=this.getYears(e);if(this.years.forEach((l,m)=>{l.dataset.year=b[m],l.dataset.date=b[m],l.textContent=b[m],l.dataset.year==r?l.setAttribute("now",""):l.removeAttribute("now"),this.minormax?l.disabled=l.dataset.date<this.min[0]||l.dataset.date>this.max[0]:l.disabled=!1,this.range?(l.dataset.date>u(this.rangedate[0],"year")&&l.dataset.date<u(this.rangedate[1],"year")?l.setAttribute("select",""):l.removeAttribute("select"),l.dataset.date==u(this.rangedate[0],"year")?l.setAttribute("selectstart",""):l.removeAttribute("selectstart"),l.dataset.date==u(this.rangedate[1],"year")?l.setAttribute("selectend",""):l.removeAttribute("selectend"),this.type!=="year"&&(this.range==="left"&&l.dataset.year>u(d.value,"year")||this.range==="right"&&l.dataset.year<u(h.value,"year"))&&(l.disabled=!0)):l.dataset.year==e?l.setAttribute("current",""):l.removeAttribute("current")}),this.switch.textContent=b[0]+"年 - "+(b[0]+19)+"年",this.switch.disabled=!0,this.minormax?(this.prev.disabled=this.min[0]>=this.years[0].dataset.year,this.next.disabled=this.max[0]<=this.years[19].dataset.year):(this.prev.disabled=!1,this.next.disabled=!1),this.range==="left"&&this.init){const l=this.nextElementSibling;this.years[19].dataset.year>=l.years[0].dataset.year&&(this.next.disabled=!0)}if(this.range==="right"&&this.init){const l=this.previousElementSibling;this.years[0].dataset.year<=l.years[19].dataset.year&&(this.prev.disabled=!0)}}}connectedCallback(){this.datePane=this.shadowRoot.getElementById("date-pane"),this.prev=this.datePane.querySelector(".prev"),this.next=this.datePane.querySelector(".next"),this.switch=this.datePane.querySelector(".date-switch"),this.dateBody=this.datePane.querySelector(".date-body"),this.dateCon=this.datePane.querySelector(".date-con"),this.dateMonth=this.datePane.querySelector(".date-month"),this.dateYear=this.datePane.querySelector(".date-year"),this.days=this.dateBody.querySelectorAll("button"),this.months=this.dateMonth.querySelectorAll("button"),this.years=this.dateYear.querySelectorAll("button"),this.mode=this.type,this.value=this.defaultvalue,this.prev.addEventListener("click",()=>{let[t,e,i]=P(this.$value);switch(this.nativeclick=!0,this.mode){case"date":this.value=new Date(...this.toDay(t,e-2,i));break;case"month":this.value=new Date(...this.toDay(t-1,e-1,i));break;case"year":this.value=new Date(...this.toDay(t-20,e-1,i))}}),this.next.addEventListener("click",()=>{let[t,e,i]=P(this.$value);switch(this.nativeclick=!0,this.mode){case"date":this.value=new Date(...this.toDay(t,e,i));break;case"month":this.value=new Date(...this.toDay(t+1,e-1,i));break;case"year":this.value=new Date(...this.toDay(t+20,e-1,i))}}),this.switch.addEventListener("click",()=>{switch(this.mode){case"date":this.mode="month",this.render();break;case"month":this.mode="year",this.render();break}}),this.dateBody.addEventListener("click",t=>{const e=t.target.closest("button");this.nativeclick=!0,e&&(this.select(e.dataset.date),this.value=e.dataset.date)}),this.dateMonth.addEventListener("click",t=>{const e=t.target.closest("button");let[i,o,r]=P(this.$value);if(this.nativeclick=!0,e)if(this.type=="date"){const a=new Date(i,e.dataset.month,0).getDate();this.mode="date",this.value=e.dataset.date+"-"+(r>a?a:r)}else this.select(e.dataset.date),this.value=e.dataset.date}),this.dateYear.addEventListener("click",t=>{const e=t.target.closest("button");let[i,o,r]=P(this.$value);if(this.nativeclick=!0,e)switch(this.type){case"date":const a=new Date(e.dataset.year,o,0).getDate();this.mode="month",this.value=e.dataset.date+"-"+o+"-"+(r>a?a:r);break;case"month":this.mode="month",this.value=e.dataset.date+"-"+o;break;default:this.select(e.dataset.date+""),this.value=e.dataset.date+"";break}}),this.init=!0}get value(){return u(this.$value,this.type)}get date(){return new Date(this.$value)}get type(){return this.getAttribute("type")||"date"}get rangedate(){return this.$rangedate||[new Date,new Date]}set rangedate(t){this.$rangedate=t}set min(t){this.setAttribute("min",t)}set max(t){this.setAttribute("max",t)}get mode(){return this.$mode||this.type}set type(t){this.setAttribute("type",t)}set mode(t){this.$mode=t,this.dateCon.dataset.type=t}set value(t){this.minormax&&(t=Math.max(Math.min(new Date(t),new Date(this.max)),new Date(this.min))),this.render(t),this.init&&(this.range==="left"&&this.nextElementSibling.render(),this.range==="right"&&this.previousElementSibling.render(),this.nativeclick&&(this.nativeclick=!1,this.dispatchEvent(new CustomEvent("change",{detail:{value:t,date:this.date}}))))}attributeChangedCallback(t,e,i){this.init&&(t=="min"&&this.datePane&&i!==null&&this.render(),t=="max"&&this.datePane&&i!==null&&this.render(),t=="type"&&this.datePane&&i!==null&&this.render())}}customElements.get("xy-date-pane")||customElements.define("xy-date-pane",xt);class wt extends HTMLElement{static get observedAttributes(){return["min","max","type"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host{
            display:inline-flex;
            font-size: 14px;
        ...
        `}focus(){this.select.focus()}connectedCallback(){this.popover=this.shadowRoot.getElementById("popover"),this.popcon=this.shadowRoot.getElementById("popcon"),this.select=this.shadowRoot.getElementById("select"),this.datetxt=this.shadowRoot.getElementById("datetxt"),this.btnCancel=this.shadowRoot.getElementById("btn-cancel"),this.btnSubmit=this.shadowRoot.getElementById("btn-submit"),this.select.addEventListener("click",()=>{this.datePane||(this.range?this.datePane=new wt:this.datePane=new xt,this.min&&(this.datePane.min=this.min),this.max&&(this.datePane.max=this.max),this.datePane.type=this.type,this.datePane.defaultvalue=this.defaultvalue,this.popcon.prepend(this.datePane))}),this.btnSubmit.addEventListener("click",()=>{this.nativeclick=!0,this.value=this.datePane.value}),this.popcon.addEventListener("close",()=>{this.datePane.value=this.value,this.datePane.mode=this.type}),this.value=this.defaultvalue}get min(){return this.getAttribute("min")}get max(){return this.getAttribute("max")}get range(){return this.getAttribute("range")!==null}get defaultvalue(){const t=this.getAttribute("defaultvalue");if(this.range)if(t){const e=t.split("~");return e[0]>e[1]?[e[1],e[0]]:e}else return[new Date,new Date];else return t||new Date}get value(){return this.range?this.$value.map(t=>u(t,this.type)):u(this.$value,this.type)}get date(){return this.range?this.$value.map(t=>new Date(t)):new Date(this.$value)}get type(){return this.getAttribute("type")||"date"}get disabled(){return this.getAttribute("disabled")!==null}get dir(){return this.getAttribute("dir")}set min(t){this.setAttribute("min",t)}set max(t){this.setAttribute("max",t)}set dir(t){this.setAttribute("dir",t)}set type(t){this.setAttribute("type",t)}set disabled(t){t===null||t===!1?this.removeAttribute("disabled"):this.setAttribute("disabled","")}set defaultvalue(t){this.setAttribute("defaultvalue",t)}set value(t){this.$value=t,this.datetxt.textContent=this.range?this.value.join("~"):this.value,this.nativeclick?(this.nativeclick=!1,this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value,date:this.date}}))):this.datePane?this.datePane.value=this.value:this.defaultvalue=this.range?this.value.join("~"):this.value}attributeChangedCallback(t,e,i){t=="disabled"&&this.select&&(i!=null?this.select.setAttribute("disabled","disabled"):this.select.removeAttribute("disabled")),t=="dir"&&this.popover&&i!=null&&(this.popover.dir=i),t=="min"&&this.datePane&&i!==null&&(this.datePane.min=i),t=="max"&&this.datePane&&i!==null&&(this.datePane.max=i),t=="type"&&this.datePane&&i!==null&&(this.datePane.type=i)}}customElements.get("xy-date-picker")||customElements.define("xy-date-picker",be);class fe extends HTMLElement{static get observedAttributes(){return["checked"]}constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host {
            display:contents;
        }
        ...
        <slot></slot>
        `}}customElements.get("xy-td")||customElements.define("xy-td",ge);class me extends HTMLElement{constructor(){super();const t=this.attachShadow({mode:"open"});t.innerHTML=`
        <style>
        :host {
            line-height: 1.5;
            display:grid;
        ...
        <slot id="con"></slot>
        `}get coord(){return this.getAttribute("coord")!==null}get draggable(){return this.getAttribute("draggable")!==null}get dragstart(){return this.getAttribute("dragstart")!==null}get resizable(){return this.getAttribute("resizable")!==null}get dragaxis(){return this.getAttribute("dragaxis")}get resizing(){return this.getAttribute("resizing")!==null}get allowdrop(){return this.getAttribute("allowdrop")!==null}get allowhover(){return this.getAttribute("allowhover")!==null}set dragstart(t){t===null||t===!1?this.removeAttribute("dragstart"):this.setAttribute("dragstart","")}set resizing(t){t===null||t===!1?this.removeAttribute("resizing"):this.setAttribute("resizing","")}connectedCallback(){const t=this.shadowRoot.getElementById("con");if(this.coord&&this.addEventListener("mousemove",e=>{const{left:i,top:o,width:r,height:a}=this.getBoundingClientRect();t.style.setProperty("--x",(e.clientX-i)/r),t.style.setProperty("--y",(e.clientY-o)/a)}),this.draggable){this.setAttribute("draggable",!0);const e=new Image;e.src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' %3E%3Cpath /%3E%3C/svg%3E";let i=0,o=0,r=0,a=0,n=this.dragaxis;this.addEventListener("dragstart",h=>{if(this.resizing)return h.preventDefault(),!1;this.dragData={},h.dataTransfer.setDragImage(e,0,0),h.dataTransfer.effectAllowed="all";const{left:d,top:c}=this.getBoundingClientRect();i=h.clientX,o=h.clientY,r=i-d,a=o-c,h.dataTransfer.setData("dragData",JSON.stringify({id:this.id,offsetX:i,offsetY:o})),this.cloneObj=document.createElement("DIV");const b=this.cloneNode(!0);b.setAttribute("fake",""),b.setAttribute("dragging",""),b.style.width=this.offsetWidth+"px",b.style.height=this.offsetHeight+"px",b.style.transform="translate3d(0,0,0)",this.cloneObj.appendChild(b),this.cloneObj.style=`position:fixed;left:0;top:0;z-index:999;pointer-events:none;transform:translate3d( ${d}px ,${c}px,0)`,document.body.appendChild(this.cloneObj)}),document.addEventListener("dragover",h=>{if(this.cloneObj){this.dragstart=!0;let d=~~(h.clientX-r),c=~~(h.clientY-a);h.shiftKey||this.dragaxis?n==="X"?c=~~(o-a):n==="Y"?d=~~(i-r):n=~~Math.abs(h.clientX-i)>~~Math.abs(h.clientY-o)&&"X"||~~Math.abs(h.clientX-i)<~~Math.abs(h.clientY-o)&&"Y"||"":n="",i=d+r,o=c+a,this.dragData.left=d,this.dragData.top=c,this.cloneObj.style.transform=`translate3d( ${d}px ,${parseInt(c)}px,0)`}}),this.addEventListener("dragend",h=>{if(this.cloneObj){const{left:d,top:c}=this.getBoundingClientRect(),b=this.cloneObj.animate([{transform:this.cloneObj.style.transform},{transform:`translate3d( ${d}px ,${c}px,0)`}],{duration:200,easing:"ease-in-out"});b.onfinish=()=>{document.body.removeChild(this.cloneObj),this.cloneObj=null,this.dragData=null,this.dragstart=!1}}})}if(this.allowdrop){let e=null;this.addEventListener("dragover",i=>{i.preventDefault()}),this.addEventListener("drop",i=>{i.preventDefault(),i.stopPropagation(),this.removeAttribute("over")}),this.addEventListener("dragleave",i=>{i.stopPropagation(),e===i.target&&this.removeAttribute("over")}),this.addEventListener("dragenter",i=>{i.stopPropagation(),e=i.target,this.setAttribute("over","")})}if(this.allowhover&&(this.addEventListener("mouseover",e=>{this.setAttribute("hover","")}),this.addEventListener("mouseout",e=>{e.stopPropagation(),this.removeAttribute("hover")})),this.resizable){const e=this.shadowRoot.querySelector(".resize");let i=0,o=0,r="",a=0,n=0,h=0,d=0,c=[0,0];this.addEventListener("click",()=>{e.focus()}),e.addEventListener("mousedown",b=>{b.stopPropagation();const l=b.path||b.composedPath&&b.composedPath(),m=getComputedStyle(this).transform;if(c=m==="none"?[0,0]:m.match(/[\d]+/g).slice(4),l[0].tagName==="I"){this.resizing=!0,i=b.pageX,o=b.pageY,r=l[0].className;const S=this.getBoundingClientRect();a=S.width,n=S.height,this.dispatchEvent(new CustomEvent("resizestart",{detail:{offsetX:0,offsetY:0,width:a,height:n}}))}}),document.addEventListener("mousemove",b=>{if(this.resizing){switch(b.stopPropagation(),window.getSelection().removeAllRanges(),h=b.pageX-i,d=b.pageY-o,r){case"tl":this.style.width=a-h+"px",this.style.height=n-d+"px";break;case"t":this.style.height=n-d+"px";break;case"tr":this.style.width=a+h+"px",this.style.height=n-d+"px";break;case"l":this.style.width=a-h+"px";break;case"r":this.style.width=a+h+"px";break;case"bl":this.style.width=a-h+"px",this.style.height=n+d+"px";break;case"b":this.style.height=n+d+"px";break;case"br":this.style.width=a+h+"px",this.style.height=n+d+"px";break}h=r.includes("l")?h:0,d=r.includes("t")?d:0,this.style.transform=`translate3d(${Number(c[0])+h}px,${Number(c[1])+d}px,0)`,this.dispatchEvent(new CustomEvent("resize",{detail:{offsetX:h,offsetY:d,width:this.offsetWidth,height:this.offsetHeight}}))}}),document.addEventListener("mouseup",b=>{this.resizing&&(this.resizing=!1,this.dispatchEvent(new CustomEvent("resizend",{detail:{offsetX:h,offsetY:d,width:this.offsetWidth,height:this.offsetHeight}})))})}}}customElements.get("xy-view")||customElements.define("xy-view",ve);class xe extends HTMLElement{constructor(){super();k(this,"setlist",e=>{if(this.visible)switch(e.key){case"ArrowUp":this.selectedIndex-=1;break;case"ArrowDown":this.selectedIndex+=1;break;case"Enter":this.dispatchEvent(new InputEvent("submit"));break}});const e=this.attachShadow({mode:"open"});e.innerHTML=`
        <style>
            :host {
                position:absolute;
                display:flex;
        ...
        `}get open(){return this.getAttribute("open")!==null}get title(){return this.getAttribute("title")||"dialog"}get type(){return this.getAttribute("type")}get oktext(){return this.getAttribute("oktext")||"ok"}get portal(){const t=this.getAttribute("portal");return t?document.querySelector(t):null}get canceltext(){return this.getAttribute("canceltext")||"cancel"}get loading(){return this.getAttribute("loading")!==null}set color(t){this.setAttribute("color",t)}set title(t){this.setAttribute("title",t)}set type(t){this.setAttribute("type",t)}set oktext(t){this.setAttribute("oktext",t)}set canceltext(t){this.setAttribute("canceltext",t)}set open(t){t===null||t===!1?this.removeAttribute("open"):(this.setAttribute("open",""),this.loading&&(this.loading=!1))}set loading(t){t===null||t===!1?this.removeAttribute("loading"):this.setAttribute("loading","")}typeMap(t){let e="",i="";switch(t){case"info":e="info-circle",i="var(--infoColor,#1890ff)";break;case"success":e="check-circle",i="var(--successColor,#52c41a)";break;case"error":e="close-circle",i="var(--errorColor,#f4615c)";break;case"warning":e="warning-circle",i="var(--waringColor,#faad14)";break;case"confirm":e="question-circle",i="var(--waringColor,#faad14)";break}return{name:e,color:i}}connectedCallback(){this.remove=!1,this.autoclose=!0,this.titles=this.shadowRoot.getElementById("title"),this.btnClose=this.shadowRoot.getElementById("btn-close"),this.btnCancel=this.shadowRoot.getElementById("btn-cancel"),this.btnSubmit=this.shadowRoot.getElementById("btn-submit"),this.dialogType=this.shadowRoot.getElementById("dialog-type"),this.input=this.shadowRoot.querySelector("xy-input"),this.clientWidth,this.shadowRoot.addEventListener("transitionend",t=>{t.propertyName==="transform"&&this.open&&(this.input?this.input.focus():this.btnSubmit.focus())}),this.shadowRoot.addEventListener("transitionend",t=>{t.propertyName==="transform"&&!this.open&&(this.remove&&document.body.removeChild(this),this.dispatchEvent(new CustomEvent("close")),this.btnActive&&this.btnActive.focus())}),this.addEventListener("wheel",t=>{t.preventDefault()}),this.btnClose.addEventListener("click",()=>{this.open=!1}),this.btnCancel.addEventListener("click",async()=>{this.dispatchEvent(new CustomEvent("cancel")),this.open=!1}),this.btnSubmit.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("submit")),!this.loading&&this.autoclose&&(this.open=!1)}),this.portal&&this.portal!==this.parentNode&&this.portal.appendChild(this)}attributeChangedCallback(t,e,i){t=="open"&&this.shadowRoot&&i!==null&&(this.btnActive=document.activeElement),t=="loading"&&this.shadowRoot&&(i!==null?this.btnSubmit.loading=!0:this.btnSubmit.loading=!1),t=="title"&&this.titles&&i!==null&&(this.titles.textContent=i),t=="oktext"&&this.btnSubmit&&i!==null&&(this.btnSubmit.textContent=i),t=="canceltext"&&this.btnCancel&&i!==null&&(this.btnCancel.textContent=i),t=="type"&&this.dialogType&&i!==null&&(this.dialogType.name=this.typeMap(i).name,this.dialogType.color=this.typeMap(i).color)}}customElements.get("xy-dialog")||customElements.define("xy-dialog",j);const we={alert:function(){const s=new j;if(document.body.appendChild(s),s.remove=!0,typeof arguments[0]=="object"){const{title:t,oktext:e,content:i,ok:o}=arguments[0];s.title=t||"Alert",s.oktext=e||"确 定",s.onsubmit=o||null,s.innerHTML=i||""}else s.title="Alert",s.oktext="确 定",s.innerHTML=arguments[0]||"",s.onsubmit=arguments[1]||null;return s.open=!0,s},info:function(){const s=new j;if(document.body.appendChild(s),s.type="info",s.remove=!0,typeof arguments[0]=="object"){const{title:t,oktext:e,content:i,ok:o}=arguments[0];s.title=t||"Info",s.oktext=e||"知道了",s.onsubmit=o||null,s.innerHTML=i||""}else s.title="Info",s.oktext="知道了",s.innerHTML=arguments[0]||"",s.onsubmit=arguments[1]||null;return s.open=!0,s},success:function(){const s=new j;if(document.body.appendChild(s),s.type="success",s.remove=!0,typeof arguments[0]=="object"){const{title:t,oktext:e,content:i,ok:o}=arguments[0];s.title=t||"Success",s.oktext=e||"知道了",s.onsubmit=o||null,s.innerHTML=i||""}else s.title="Success",s.oktext="知道了",s.innerHTML=arguments[0]||"",s.onsubmit=arguments[1]||null;return s.open=!0,s},error:function(){const s=new j;if(document.body.appendChild(s),s.type="error",s.remove=!0,typeof arguments[0]=="object"){const{title:t,oktext:e,content:i,ok:o}=arguments[0];s.title=t||"Error",s.oktext=e||"知道了",s.onsubmit=o||null,s.innerHTML=i||""}else s.title="Error",s.oktext="知道了",s.innerHTML=arguments[0]||"",s.onsubmit=arguments[1]||null;return s.open=!0,s},warning:function(){const s=new j;if(document.body.appendChild(s),s.type="warning",s.remove=!0,typeof arguments[0]=="object"){const{title:t,oktext:e,content:i,ok:o}=arguments[0];s.title=t||"Warning",s.oktext=e||"知道了",s.onsubmit=o||null,s.innerHTML=i||""}else s.title="Warning",s.oktext="知道了",s.innerHTML=arguments[0]||"",s.onsubmit=arguments[1]||null;return s.open=!0,s},confirm:function(){const s=new j;if(document.body.appendChild(s),s.remove=!0,s.btnCancel.style.visibility="visible",typeof arguments[0]=="object"){const{type:t,title:e,content:i,oktext:o,canceltext:r,ok:a,cancel:n}=arguments[0];s.type=t||"confirm",s.title=e||"Confirm",s.oktext=o||"确 定",s.canceltext=r||"取 消",s.innerHTML=i||"",s.onsubmit=a||null,s.oncancel=n||null}else s.type="confirm",s.title="Confirm",s.oktext="确 定",s.canceltext="取 消",s.innerHTML=arguments[0]||"",s.onsubmit=arguments[1]||null,s.oncancel=arguments[2]||null;return s.open=!0,s},prompt:function(){const s=new j({type:"prompt"});if(document.body.appendChild(s),s.type="prompt",s.remove=!0,s.autoclose=!1,typeof arguments[0]=="object"){const{title:t,content:e,oktext:i,canceltext:o,ok:r,cancel:a}=arguments[0];s.title=t||"Prompt",s.oktext=i||"确 定",s.canceltext=o||"取 消",s.innerHTML=e||"",s.input.onsubmit=s.onsubmit=()=>{const n=s.input.value;n?(r&&r(n),s.open=!1):(XyMessage.error("内容不能为空"),s.input.focus())},s.oncancel=a||null}else s.title="Prompt",s.oktext="确 定",s.canceltext="取 消",s.innerHTML=arguments[0]||"",s.input.onsubmit=s.onsubmit=()=>{const t=s.input.value;t?(arguments[1]&&arguments[1](t),s.open=!1):(XyMessage.error("内容不能为空"),s.input.focus())},s.oncancel=arguments[2]||null;return s.open=!0,s}};window.XyDialog=we;window.XyMessage=J;var f;(function(s){s.LOAD="LOAD",s.EXEC="EXEC",s.FFPROBE="FFPROBE",s.WRITE_FILE="WRITE_FILE",s.READ_FILE="READ_FILE",s.DELETE_FILE="DELETE_FILE",s.RENAME="RENAME",s.CREATE_DIR="CREATE_DIR",s.LIST_DIR="LIST_DIR",s.DELETE_DIR="DELETE_DIR",s.ERROR="ERROR",s.DOWNLOAD="DOWNLOAD",s.PROGRESS="PROGRESS",s.LOG="LOG",s.MOUNT="MOUNT",s.UNMOUNT="UNMOUNT"})(f||(f={}));const ke=(()=>{let s=0;return()=>s++})(),Ae=new Error("ffmpeg is not loaded, call `await ffmpeg.load()` first"),Ee=new Error("called FFmpeg.terminate()");var $,_,q,W,V,rt,R;class Ce{constructor(){N(this,$,null);N(this,_,{});N(this,q,{});N(this,W,[]);N(this,V,[]);k(this,"loaded",!1);N(this,rt,()=>{p(this,$)&&(p(this,$).onmessage=({data:{id:t,type:e,data:i}})=>{switch(e){case f.LOAD:this.loaded=!0,p(this,_)[t](i);break;case f.MOUNT:case f.UNMOUNT:case f.EXEC:case f.FFPROBE:case f.WRITE_FILE:case f.READ_FILE:case f.DELETE_FILE:case f.RENAME:case f.CREATE_DIR:case f.LIST_DIR:case f.DELETE_DIR:p(this,_)[t](i);break;case f.LOG:p(this,W).forEach(o=>o(i));break;case f.PROGRESS:p(this,V).forEach(o=>o(i));break;case f.ERROR:p(this,q)[t](i);break}delete p(this,_)[t],delete p(this,q)[t]})});N(this,R,({type:t,data:e},i=[],o)=>p(this,$)?new Promise((r,a)=>{const n=ke();p(this,$)&&p(this,$).postMessage({id:n,type:t,data:e},i),p(this,_)[n]=r,p(this,q)[n]=a,o==null||o.addEventListener("abort",()=>{a(new DOMException(`Message # ${n} was aborted`,"AbortError"))},{once:!0})}):Promise.reject(Ae));k(this,"load",({classWorkerURL:t,...e}={},{signal:i}={})=>(p(this,$)||(et(this,$,t?new Worker(new URL(t,import.meta.url),{type:"module"}):new Worker(new URL("/video-2-ascii-code-style/assets/worker-DYSz7Krg.js",import.meta.url),{type:"module"})),p(this,rt).call(this)),p(this,R).call(this,{type:f.LOAD,data:e},void 0,i)));k(this,"exec",(t,e=-1,{signal:i}={})=>p(this,R).call(this,{type:f.EXEC,data:{args:t,timeout:e}},void 0,i));k(this,"ffprobe",(t,e=-1,{signal:i}={})=>p(this,R).call(this,{type:f.FFPROBE,data:{args:t,timeout:e}},void 0,i));k(this,"terminate",()=>{const t=Object.keys(p(this,q));for(const e of t)p(this,q)[e](Ee),delete p(this,q)[e],delete p(this,_)[e];p(this,$)&&(p(this,$).terminate(),et(this,$,null),this.loaded=!1)});k(this,"writeFile",(t,e,{signal:i}={})=>{const o=[];return e instanceof Uint8Array&&o.push(e.buffer),p(this,R).call(this,{type:f.WRITE_FILE,data:{path:t,data:e}},o,i)});k(this,"mount",(t,e,i)=>{const o=[];return p(this,R).call(this,{type:f.MOUNT,data:{fsType:t,options:e,mountPoint:i}},o)});k(this,"unmount",t=>{const e=[];return p(this,R).call(this,{type:f.UNMOUNT,data:{mountPoint:t}},e)});k(this,"readFile",(t,e="binary",{signal:i}={})=>p(this,R).call(this,{type:f.READ_FILE,data:{path:t,encoding:e}},void 0,i));k(this,"deleteFile",(t,{signal:e}={})=>p(this,R).call(this,{type:f.DELETE_FILE,data:{path:t}},void 0,e));k(this,"rename",(t,e,{signal:i}={})=>p(this,R).call(this,{type:f.RENAME,data:{oldPath:t,newPath:e}},void 0,i));k(this,"createDir",(t,{signal:e}={})=>p(this,R).call(this,{type:f.CREATE_DIR,data:{path:t}},void 0,e));k(this,"listDir",(t,{signal:e}={})=>p(this,R).call(this,{type:f.LIST_DIR,data:{path:t}},void 0,e));k(this,"deleteDir",(t,{signal:e}={})=>p(this,R).call(this,{type:f.DELETE_DIR,data:{path:t}},void 0,e))}on(t,e){t==="log"?p(this,W).push(e):t==="progress"&&p(this,V).push(e)}off(t,e){t==="log"?et(this,W,p(this,W).filter(i=>i!==e)):t==="progress"&&et(this,V,p(this,V).filter(i=>i!==e))}}$=new WeakMap,_=new WeakMap,q=new WeakMap,W=new WeakMap,V=new WeakMap,rt=new WeakMap,R=new WeakMap;var pt;(function(s){s.MEMFS="MEMFS",s.NODEFS="NODEFS",s.NODERAWFS="NODERAWFS",s.IDBFS="IDBFS",s.WORKERFS="WORKERFS",s.PROXYFS="PROXYFS"})(pt||(pt={}));const Le=["█","▓","▒","░","■","□","▢","▣","▤","▥","▦","▧","▨","▩","▪","▫","◼","◻","◾","◽","◦","●","○","◌","◍","◎","◯","⚬","∙","·","⋅"," "],bt=["@","#","8","B","&","W","M","b","d","p","q","Q","D","O","a","e","o","P","g","9","0","C","J","L","7","T","c","v","n","r","."," "],Re=["✸","✷","✶","✵","✴","✳","✲","✱","✰","✯","✮","✭","✬","✫","✪","✩","✧","✦","★","☆","✿","❀","❁","❂","❃","❇","❈","❉","❊","❋","·"," "],ze=["█","▓","▒","░","▇","▆","▅","▄","▃","▂","▁","◘","◙","◚","◛","◕","◔","◓","◒","◑","◐","☗","☖","☕","♨","♦","♠","♣","♥","♡","•"," "],$e=["█","▓","▒","░","▞","▚","▙","▟","▜","▛","▌","▐","▄","▀","◘","◙","◚","◛","◕","◔","◑","◒","⦿","◉","◎","○","∘","∙","·","⋅","˙"," "],M={getTimeFormat:function(s){const t=h=>h<10?"0"+h:h,e=s.getFullYear(),i=t(s.getMonth()+1),o=t(s.getDate()),r=t(s.getHours()),a=t(s.getMinutes()),n=t(s.getSeconds());return`${e}-${i}-${o} ${r}:${a}:${n}`},appendLogRecord:function(s){const t=document.querySelector(".log-content"),e=document.querySelector(".log-lines"),i=document.getElementById("logView");let o=t.innerHTML.split("<br"),r=o.length,a=!1;r>=200&&(t.innerHTML=o.slice(100).join(`<br />
`),r=t.innerHTML.split("<br"),a=!0),t.innerHTML+=`[${M.getTimeFormat(new Date)}]:${s}<br />`;const n=t.innerHTML.split("<br").length-1;e.innerHTML=Array(n).fill(0).map((h,d)=>d+1).join("<br />"),i.scrollTop=i.scrollHeight,a&&M.appendLogRecord("日志超过200条，自动清除前100条")},hiddenLog:function(){const s=document.getElementById("displayLog"),t=document.getElementById("logView"),e=document.getElementById("clearLog");t.style.display="none",e.style.display="none",s.innerText="显示日志"},showLog:function(){const s=document.getElementById("displayLog"),t=document.getElementById("logView"),e=document.getElementById("clearLog");t.style.display="block",e.style.display="inline",s.innerText="隐藏日志"},clearLog:function(){const s=document.querySelector(".log-content");s.innerHTML="";const t=document.querySelector(".log-lines");t.innerHTML="1<br />"},invertColor:function(s){s=s.replace("#","");let t=parseInt(s.substr(0,2),16),e=parseInt(s.substr(2,2),16),i=parseInt(s.substr(4,2),16);return t=255-t,e=255-e,i=255-i,"#"+((1<<24)+(t<<16)+(e<<8)+i).toString(16).slice(1)},imageToASCIICode:function(s,t,e,i,o){const r=new Blob([s.buffer],{type:"image/jpeg"}),a=new Image,n=URL.createObjectURL(r);return a.src=n,new Promise(h=>{a.onload=()=>{let d=bt;switch(e){case"几何形状":d=Le;break;case"字母数字":d=bt;break;case"特殊字符":d=Re;break;case"混合符号":d=ze;break;case"渐变符号":d=$e;break;default:d=i;break}const c=document.createElement("canvas"),b=c.getContext("2d");c.width=a.width,c.height=a.height,b.drawImage(a,0,0);const m=b.getImageData(0,0,c.width,c.height).data;let S=0;const I=Array(c.height).fill().map(()=>Array(c.width).fill(0)),D=Array(c.height).fill().map(()=>Array(c.width).fill(0));for(let E=0;E<c.height;E++)for(let v=0;v<c.width;v++){const x=(E*c.width+v)*4,H=m[x],K=m[x+1],F=m[x+2],Z=.299*H+.587*K+.114*F,Q=Math.floor(Z/d.length);S+=Z,I[E][v]=Math.min(Q,d.length-1),D[E][v]="#"+((1<<24)+(H<<16)+(K<<8)+F).toString(16).slice(1)}const T=S/(c.width*c.height),A=document.createElement("canvas"),C=A.getContext("2d");A.width=c.width*10,A.height=c.height*10,C.font="10px monospace",C.textBaseline="center",C.textBaseline="middle";let X="white",O="black";T>255*.58&&(X="black",O="darkgray");for(let E=0;E<I.length;E++)for(let v=0;v<I[E].length;v++){const x=I[E][v],H=d[x];t||(T>o?X=M.invertColor(D[E][v]):X=D[E][v]),C.fillStyle=O,C.fillRect(v*10,E*10,10,10),C.fillStyle=X,C.fillText(H,v*10+5,E*10+5)}const y=document.getElementById("progress-after"),z=y.getContext("2d");y.width=A.width,y.height=A.height,z.drawImage(A,0,0);const L=document.getElementById("progress-before"),st=L.getContext("2d");L.width=A.width,L.height=A.height,st.drawImage(c,0,0,A.width,A.height),URL.revokeObjectURL(n),A.toBlob(E=>{E.arrayBuffer().then(v=>{const x=new Uint8Array(v);h(x)})},"image/jpeg")}}).finally(()=>{URL.revokeObjectURL(n)})}},Se="/video-2-ascii-code-style/assets/ffmpeg-core-CI9Irx9p.js",Ie="/video-2-ascii-code-style/assets/ffmpeg-core-CgUfceKH.wasm",Be=s=>new Promise((t,e)=>{const i=new FileReader;i.onload=()=>{const{result:o}=i;o instanceof ArrayBuffer?t(new Uint8Array(o)):t(new Uint8Array)},i.onerror=o=>{var r,a;e(Error(`File could not be read! Code=${((a=(r=o==null?void 0:o.target)==null?void 0:r.error)==null?void 0:a.code)||-1}`))},i.readAsArrayBuffer(s)}),Te=async s=>{let t;if(typeof s=="string")/data:_data\/([a-zA-Z]*);base64,([^"]*)/.test(s)?t=atob(s.split(",")[1]).split("").map(e=>e.charCodeAt(0)):t=await(await fetch(s)).arrayBuffer();else if(s instanceof URL)t=await(await fetch(s)).arrayBuffer();else if(s instanceof File||s instanceof Blob)t=await Be(s);else return new Uint8Array;return new Uint8Array(t)},{appendLogRecord:g}=M;document.addEventListener("DOMContentLoaded",()=>{const s=document.getElementById("chooseVideo"),t=document.getElementById("convertBtn"),e=document.getElementById("displayLog");document.getElementById("logView");const i=document.getElementById("clearLog"),o=document.getElementById("fps"),r=document.getElementById("style"),a=document.getElementById("fpsDefaultSwitch"),n=document.getElementById("styleDefaultSwitch"),h=document.getElementById("progress-after"),d=document.getElementById("progress-before"),c=document.getElementById("player"),b=document.getElementById("colorfulSwitch"),l=document.getElementById("customView"),m=document.getElementById("customSymbols"),S=document.getElementById("inputCount"),I=document.getElementById("threshold"),D=document.getElementById("thresholdValue");let T=null,A=!1,C=0,X="",O="";I.value=1480,I.onchange=z=>{D.innerText=parseInt(z.currentTarget.value)/10},m.onchange=z=>{S.innerText=z.currentTarget.value.length},r.onchange=z=>{z.detail.value=="自定义符号"?l.style.display="block":l.style.display="none"},n.checked=!0,a.checked=!0;const y=new Ce;y.on("log",({message:z})=>{if(g(`${O}:ffmpeg:${z}`),z.includes("/")){let L=z.split("/");if(L.length!=2||!parseInt(L[0])||!parseInt(L[1]))return;C=parseInt(parseInt(L[0])/parseInt(L[1]))}}),i.addEventListener("click",()=>{M.clearLog()}),e.addEventListener("click",()=>{A=!A,A?M.showLog():M.hiddenLog()}),s.addEventListener("click",async z=>{try{const[L]=await window.showOpenFilePicker({types:[{description:"请选择视频文件",accept:{"video/*":[".mp4",".avi",".mov",".wmv",".flv",".mkv"]}}],multipe:!1});T=await L.getFile(),T&&(t.disabled=!1),g(`选择视频文件:${T.name}`)}catch(L){g(`选择视频文件失败:${L}`)}finally{A=!0,M.showLog()}}),t.addEventListener("click",async z=>{const L=b.checked,st=r.value,E=I.value/10;let v=[];if(st=="自定义符号"&&(v=m.value.split(""),v.length<4||v.length>256||!(256%v.length))){J.error("自定义符号不合法"),g("自定义符号不合法");return}if(!T){J.error("请选择视频文件"),g("请选择视频文件");return}t.disabled=!0,c.style.display="none",A=!0,c.pause();try{URL.revokeObjectURL(X)}catch{}M.showLog();try{const x="/source",H=`${x}/${T.name}`,K=`${x}/audio.mp3`,F=`${x}/tempImg`,Z=`${x}/asciiImg`;y.terminate(),y.loaded||(g("初始化ffmpeg库"),await y.load({coreURL:Se,wasmURL:Ie})),g("删除临时文件夹");try{await y.deleteDir(x)}catch{}g("创建临时文件夹"),await y.createDir(x),g("创建图片文件夹"),await y.createDir(F),g(`加载文件:${T.name},到ffmpeg`),await y.writeFile(H,await Te(T)),g("提取视频帧率信息"),O="获取视频帧信息",await y.ffprobe(["-v","error","-select_streams","v:0","-show_entries","stream=r_frame_rate","-of","default=noprint_wrappers=1:nokey=1",H]),g(`视频fps:${C}`);let Q=0,nt=0,at=0;for(let w=3;w<=C;w++)if(!Q)C%w==0&&(Q=w,w=parseInt(C/4)-1);else if(!nt)C%w==0&&(nt=w,w=parseInt(C/2)-1);else if(!at)C%w==0&&(at=w);else break;for(;!C;);let Y="10";switch(o.value){case"低":Y=String(Q);break;case"高":Y=String(at);break;case"中":default:Y=String(nt);break}g(`按${Y}帧提取视频每帧图片`),O="提取视频帧图片",await y.exec(["-i",H,"-r",Y,"-vf","scale=100:-1","-v","info",F+"/img_%05d.jpg"]),g("提取视频帧完成"),g("提取视频音频文件"),O="提取视频音频文件",await y.exec(["-i",H,"-vn","-acodec","libmp3lame",K]),g("提取视频音频文件完成"),h.style.display="inline",d.style.display="inline",g("开始处理图片数据"),g("图片处理进度0.01%");const ot=await y.listDir(F),kt=ot.length-2;await y.createDir(Z);for(let w=2;w<ot.length;w++){w%parseInt(Y)*5||g(`图片处理进度${(w/kt*100).toFixed(2)}%`);const Ct=await y.readFile(`${F}/${ot[w].name}`),Lt=await M.imageToASCIICode(Ct,!L,st,v,E);await y.writeFile(`${Z}/${ot[w].name}`,Lt)}g("图片处理进度100%"),h.style.display="none",d.style.display="none",g("重新合成视频"),O="重新合成视频";let tt=["-framerate",Y,"-i",`${Z}/img_%05d.jpg`];try{y.readFile(K)&&(tt=tt.concat(["-i",K]))}catch{}tt=tt.concat(["-c:v","libx264","-pix_fmt","yuv420p","-c:a","aac","-strict","-2","-y",`${x}/output.mp4`]),await y.exec(tt),g("视频合成完成");const At=await y.readFile(`${x}/output.mp4`),Et=new Blob([At.buffer],{type:"video/mp4"});for(X=window.URL.createObjectURL(Et),g("开始播放视频"),A=!1,M.hiddenLog(),c.style.display="block";c.firstChild;)c.removeChild(c.firstChild);const lt=document.createElement("source");lt.src=X,lt.type="video/mp4",c.appendChild(lt),c.load(),c.play()}catch(x){String(x).indexOf("RuntimeError: memory access out of bounds")>-1?(g("转换异常:换一个小一点的视频或者切换帧率再试"),J.error("转换异常:换一个小一点的视频或者切换帧率再试")):(g("转换异常:"+x),J.error("转换异常")),console.error(x.stack)}finally{t.disabled=!1,g("转换结束"),h.style.display="none",d.style.display="none"}})});
